...
# Stage4 RJ45 image preserving forced UART login.
# It starts from the forced-login UART image, applies RJ45 power settings, moves
# PCIe reset to schematic GPIO3_D1, removes hotplug/presence GPIOs, and enables
# the tested PCIe path. SRC is copied first; only DST is modified.
...
DST="$IMGDIR/opi5p-rj45-stage4-perst-gpio3d1-nohotplug-forcedlogin115200.img"
OFFSET=$((61440 * 512))
...
echo "Creating stage4 RJ45 image with forced UART login..."
rm -f "$DST"
...
echo "Keep RJ45 3.3V power on GPIO3_B4 active-high."
...
echo "Use RTL8125 PERST from schematic: PCIE20_1_2_PERSTN_L -> GPIO3_D1."
