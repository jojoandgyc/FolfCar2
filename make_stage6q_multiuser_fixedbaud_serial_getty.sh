...
DST="$IMGDIR/opi5p-rj45-stage6q-fe180000-phy2-rj45fix-uart039-login1500000-multiuser.img"
OFFSET=$((61440 * 512))
...
echo "== stage6q verify =="
echo "Image: $DST"
echo "GETTY_TARGET_WANTS"
find "$MNT/etc/systemd/system/getty.target.wants" -maxdepth 1 -printf "%f -> %l\n" | sort
echo "MULTI_USER_TARGET_WANTS"
find "$MNT/etc/systemd/system/multi-user.target.wants" -maxdepth 1 -name 'serial-getty@ttyS*.service' -printf "%f -> %l\n" | sort
for tty in ttyS0 ttyS3 ttyS9; do
	echo "--$tty override"
