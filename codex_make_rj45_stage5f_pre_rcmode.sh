...
SRC_IMG="$IMGDIR/opi5p-rj45-stage5e-after-hwio-diag-forcedlogin115200.img"
DST_IMG="$IMGDIR/opi5p-rj45-stage5f-pre-rcmode-before-dbi-forcedlogin115200.img"
PATCH="$BUILD/codex_pcie_stage5f_pre_rcmode.patch"
OFFSET=$((61440 * 512))
...
if ! grep -q "codex pcie diag5f: pre-host APB RC mode start" drivers/pci/controller/dwc/pcie-dw-rockchip.c; then
	echo "Checking PCIe stage5f pre-RC-mode patch..."
	git apply --check "$PATCH"
	echo "Applying PCIe stage5f pre-RC-mode patch..."
	git apply "$PATCH"
else
	echo "PCIe stage5f pre-RC-mode patch already present."
fi
...
grep -n "codex pcie diag5f" drivers/pci/controller/dwc/pcie-dw-rockchip.c
